...
OUTPUT_ROOT="${REPO_DIR}/output/eval_rerank_onoff_refcocog"
...
run_mode() {
  ...
  python "${REPO_DIR}/train_net.py" \
    ...
    MODEL.OpenWorldSAM2.FUSION.EVIDENCE_RERANK "${rerank_flag}" \
...
printf "%-12s %-10s %-10s %-10s %-10s %-10s\n" "------------" "----------" "----------" "----------" "----------" "----------"
run_mode "rerank_off" "False"
run_mode "rerank_on" "True"
