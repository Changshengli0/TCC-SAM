...
OUTPUT_ROOT="${REPO_DIR}/output/eval_fallback_thr_sweep_refcocog"
...
printf "%-10s %-10s %-10s %-10s %-10s %-10s\n" "threshold" "cIoU" "mIoU" "p@0.5" "p@0.7" "p@0.9"
printf "%-10s %-10s %-10s %-10s %-10s %-10s\n" "----------" "----------" "----------" "----------" "----------" "----------"

for thr in "${THRS[@]}"; do
  log_file="${OUTPUT_ROOT}/thr_${thr}.log"
  run_dir="${OUTPUT_ROOT}/thr_${thr}"
  mkdir -p "${run_dir}"
  ...
  python "${REPO_DIR}/train_net.py" \
    ...
    MODEL.OpenWorldSAM2.FUSION.WEAK_MAP_FALLBACK_THR "${thr}" \
