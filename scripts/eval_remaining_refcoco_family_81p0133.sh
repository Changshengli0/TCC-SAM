...
for DATASET in "${DATASETS[@]}"; do
  SAFE_NAME=$(echo "${DATASET}" | sed 's/+/_plus_/g' | sed 's/[^A-Za-z0-9_]/_/g')
  OUT=./output/eval_81p0133_${SAFE_NAME}
...
echo ""
echo "================ Remaining RefCOCO family summary ================"
for DATASET in "${DATASETS[@]}"; do
  SAFE_NAME=$(echo "${DATASET}" | sed 's/+/_plus_/g' | sed 's/[^A-Za-z0-9_]/_/g')
  LOG=output/eval_81p0133_${SAFE_NAME}/run_0/log.txt
  echo "----- ${DATASET} -----"
